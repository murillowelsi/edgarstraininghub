...
const WorkoutDetail: React.FC<{ workout: any, onStart: () => void, onBack: () => void }> = ({ workout, onStart, onBack }) => {
  ...
  const getEquipmentIcon = (name: string) => {
    ...
    if (lowerCaseName.includes('body')) {
      return <BodyWeightIcon className="w-10 h-10 text-gray-600" />;
    }
    if (lowerCaseName.includes('mat')) {
      return <MatIcon className="w-10 h-10 text-gray-600" />;
    }
    return <Dumbbell className="w-10 h-10 text-gray-600" />;
  };
    ...
    <div className="bg-white text-gray-900 flex flex-col h-screen">
      ...
      <main className="flex-1 overflow-y-auto pb-32 px-6">
        ...
        <div className="flex items-center gap-4 pt-6 mb-6">
          <div className="w-8 h-8 rounded-full border-4 border-blue-400 flex-shrink-0" />
        ...
        <div className="space-y-3 mb-8">
          ...
          <div className="flex items-center gap-3 text-gray-700">
            ...
            <span className="text-base">{workout.exercises?.length || 0} Exercises</span>
          </div>
        </div>
        ...
        <div className="mb-8">
          ...
          <div className="flex gap-4">
            {workout.equipment?.map((item, index) => (
              <div key={index} className="flex flex-col items-center gap-2 bg-gray-100 rounded-2xl p-4 min-w-[80px]">
                {getEquipmentIcon(item)}
                <span className="text-sm text-gray-700 text-center font-medium">{item}</span>
              </div>
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-700 leading-relaxed">{workout.instructions}</p>
        </div>
